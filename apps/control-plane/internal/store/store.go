...
type Server struct {
	...
	OwnerID *uuid.UUID
	Name    string
	// Game = id ของ game definition (internal/games) — Variant คือ variant ภายในเกมนั้น
	Game    string
	Variant string
	// GameVersion = เวอร์ชันของเกม (ชื่อ field/คอลัมน์เดิมที่คงไว้เพื่อไม่ break contract)
	GameVersion string
	MemoryMB    int
	HostPort    *int
	Status      string
