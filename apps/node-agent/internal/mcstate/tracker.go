// Package mcstate ติดตามสถานะ "ภายในเกม" ของแต่ละ instance (ผู้เล่นออนไลน์, metric ประจำเกม)
...
//   - MetricCommand (minecraft = `tps`) ยิงไปพร้อมกันทุกรอบ (ค่าต้อง refresh) ถ้ารอบแรก
...
type Snapshot struct {
	Online     []string
	MaxPlayers int
	// Metric = ค่าประจำเกมที่วัดจาก console (minecraft = TPS)
...
type serverState struct {
	...
	stop         chan struct{}
	// ready ปิดเมื่อเห็น event ready = world โหลดเสร็จ ยิงคำสั่งเข้า console ได้ปลอดภัย —
...
func (t *Tracker) OnAttach(serverID string) {
	...
	if !ok || spec.Parse == nil {
		log.Printf("mcstate: no console spec for server=%s, in-game state disabled", serverID)
		return
	}
...
func (t *Tracker) pollLoop(serverID string, stop <-chan struct{}, ready <-chan struct{}) {
	// รอ server ประกาศ start เสร็จก่อนยิงคำสั่งแรก — ยิงตอน world ยังโหลดไม่เสร็จ
...
func (t *Tracker) poll(serverID string) {
	...
	if roster != "" {
		if err := writer.WriteInput(serverID, roster); err != nil {
			log.Printf("mcstate roster command failed: server=%s err=%v", serverID, err)
			return
		}
	}
	if metric != "" {
		if err := writer.WriteInput(serverID, metric); err != nil {
			log.Printf("mcstate metric command failed: server=%s err=%v", serverID, err)
		}
	}
}
