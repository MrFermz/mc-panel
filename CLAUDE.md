...
login — ค่าที่มี `@` = email account, ไม่มี = username-only account; + password สุ่ม 20 ตัว
...
(`lib/user-display.ts`) = display_name → username → email — อย่า inline fallback chain เอง
...
- web: เมนู/ปุ่มแสดงตาม effective cap ผ่าน `hasCapability` (`lib/capabilities.ts` ต้อง sync กับ catalog)
- UI จัดการสิทธิ์อยู่ที่ **หน้า** `/admin/users/{id}/permissions` (ไม่ใช่ modal) — role preset
  (`lib/user-roles.ts`, เป็นแค่ทางลัดฝั่ง UI backend ไม่รู้จัก "role") + toggle รายข้อจัดกลุ่มตาม feature
...
ขาเข้า: WS `{"type":"input"}` → เช็ค cap `console.write` + `can_console_write` ต่อ message
...
ก่อนแตะ filesystem. ต้องมี cap `files.view/write/delete` ตาม action **และ** `can_manage_files`
(หรือ owner/admin). REST อยู่ `/api/servers/{id}/files*` (ดู docs/api.md)
