...
| DELETE | `/api/servers/{id}` | owner/admin | → `{job}` — สั่งงาน delete (ลบ container + ข้อมูลทั้งหมด); ต้อง stopped/errored เท่านั้น (409 `invalid_state` — หยุด server ก่อนถึงจะลบได้) |
