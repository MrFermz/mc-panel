...
| GET | `/api/servers/{id}/permissions` | owner/admin | → `{permissions: [{user_id, email, username, display_name, avatar_url, role, capabilities}]}` — `role` = `owner`\|`member`, `capabilities` = server-scoped cap ที่ grant (owner ว่าง = ได้ทุกอย่าง) |
| POST | `/api/servers/{id}/permissions` | owner/admin | `{user_id?, email?, role, capabilities?}` → upsert; `role` = `owner`\|`member` (400 `invalid_role`); `member` ต้องส่ง `capabilities[]` ที่เป็น server-scoped ทั้งหมด (400 `invalid_capability`), `owner` เก็บ capabilities ว่าง; resolve target ด้วย `user_id` ก่อน (จาก `/users/directory`) ไม่งั้น fallback `email` (404 `user_not_found`); demote owner คนสุดท้าย → 409 `last_owner` |
