...
การ "เห็น" server (อยู่ใน list/switcher) = มี `server_permissions` row (role ใดก็ได้) หรือ
`servers.view_all`/admin. web มี role preset ต่อ server (owner/operator/moderator/viewer/custom)
เป็นทางลัด UI — backend รู้จักแค่ role + capabilities[]
...
| DELETE | `/api/servers/{id}` | `servers.delete` | → `{job}` — สั่งงาน delete (ลบ container + ข้อมูลทั้งหมด); ต้อง stopped/errored เท่านั้น (409 `invalid_state` — หยุด server ก่อนถึงจะลบได้) |
