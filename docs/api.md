...
| GET | `/api/users/directory` | login แล้ว | → `{users: [{id, email, username, display_name}]}` — user ที่ active + ยังไม่ถูกลบทั้งหมด (field เบา, ไม่ leak สิทธิ์/สถานะ); **ไม่ต้องมี** `users.manage` — owner ใช้เลือก collaborator ตอน grant permission (`email` อาจเป็น `""`, `username` อาจเป็น null); เรียงตาม username/email |
...
| Method | Path | Body → Response |
|---|---|---|
| GET | `/api/servers/{id}/players` | → `{whitelist_enabled, players: [{uuid, username, whitelisted, seen, op, banned}]}` (unified, เรียงตาม username) |
...
`stats`: `{cpu_percent, memory_used_mb, memory_limit_mb, net_rx_bps, net_tx_bps, disk_read_bps, disk_write_bps, updated_at}` หรือ `null` — resource monitoring แบบ realtime
