...
.PHONY: down
down: mc-stop ## ปิด full stack + MC instance ทั้งหมด (data ยังอยู่)
	$(FULL_COMPOSE) down
...
.PHONY: mc-stop
mc-stop: ## หยุด MC instance ทุกตัวบนเครื่องนี้ (container ที่ agent จัดการ)
	@ids=$$(docker ps -q --filter "label=mc.managed_by=mc-panel-agent"); \
	if [ -n "$$ids" ]; then \
		echo "หยุด MC instance: $$(echo $$ids | wc -w | tr -d ' ') ตัว"; \
		docker stop -t 40 $$ids > /dev/null; \
	else \
		echo "ไม่มี MC instance ที่รันอยู่"; \
	fi
...
admin-reset-password: ## รีเซ็ต password ของ admin (ADMIN_USERNAME) กรณีลืม — สุ่มใหม่ + พิมพ์ครั้งเดียว
...
.PHONY: dev
dev: ## เปิด dev ทั้ง stack ใน container พร้อม hot reload (control-plane/agent=air, web=next dev)
	@test -f .env || $(MAKE) env
	$(DEV_APP_COMPOSE) up -d --remove-orphans
	@echo ""
	@echo "dev พร้อม (hot reload): http://localhost:3000"
	@echo "ครั้งแรก control-plane/web ต้อง compile/install — รอสัก 1-2 นาที (ดูความคืบหน้าที่ make dev-logs)"
	@echo "ดู log รวม: make dev-logs   |   ปิด: make dev-down"

.PHONY: dev-logs
dev-logs: ## ดู log รวมของ dev container (เห็น air rebuild / next dev แบบ real-time)
	$(DEV_APP_COMPOSE) logs -f

.PHONY: dev-down
dev-down: mc-stop ## ปิด dev container + MC instance ทั้งหมด (data ใน infra ยังอยู่)
...
purge: ## ล้างทั้งหมด (data, volume, binary, node_modules) ต้องพิมพ์ยืนยัน
	...
	@echo "  - MC container ทุกตัวบนเครื่องนี้ (world ใน data/servers ยังอยู่)"
	...
	if [ -n "$$ids" ]; then docker rm -f $$ids > /dev/null; echo "ลบ MC container ที่ค้างแล้ว"; fi
	rm -rf bin/
